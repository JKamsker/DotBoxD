...
public sealed partial class SandboxHost
{
        ...
        CompiledNoAuditRunState? reusableNoAuditState)
    {
        try
        {
            return await ExecuteCompiledPreparedValueCoreAsync(
                    plan,
                    entrypoint,
                    input,
                    options,
    ...
    private async ValueTask<PreparedExecutionResult> ExecuteCompiledPreparedValueCoreAsync(
        ExecutionPlan plan,
        string entrypoint,
        SandboxValue input,
        SandboxExecutionOptions options,
        ...
        CompiledNoAuditRunState? reusableNoAuditState)
    {
        ...
        {
            return await ExecuteCompiledPreparedWithAuditAsync(
                    executable,
                    plan,
                    entrypoint,
                    input,
        ...
        return ExecuteCompiledPreparedNoAuditValue(
            executable,
            plan,
            entrypoint,
            input,
    ...
    private async ValueTask<PreparedExecutionResult> ExecuteCompiledPreparedWithAuditAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        var fullResult = await execution.ConfigureAwait(false);
        return PreparedExecutionResult.FromResult(Publish(fullResult));
    }

    private PreparedExecutionResult ExecuteCompiledPreparedNoAuditValue(
        CompiledExecutable executable,
        ExecutionPlan plan,
        string entrypoint,
        SandboxValue input,
