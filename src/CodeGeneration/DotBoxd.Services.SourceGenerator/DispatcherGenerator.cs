...
internal static class DispatcherGenerator
{
    public static string Generate(ServiceModel service, CancellationToken ct = default)
    {
        ...
        var dispatcherInterfaces = CanDispatchWithoutStreaming(service)
            ? "global::DotBoxd.Services.Server.IServiceDispatcher, global::DotBoxd.Services.Server.INonStreamingServiceDispatcher"
            : "global::DotBoxd.Services.Server.IServiceDispatcher";
        ...
        CancellationToken ct)
    {
        ...
        if (isInstanceScoped)
        {
            ...
            sb.AppendLine("            DispatchOnInstanceAsync(instanceId, method, payload, serializer, registry, output, global::DotBoxd.Services.Streaming.RpcStreamingContext.Disabled, ct);");
            sb.AppendLine();
        ...
        else
        {
            ...
            sb.AppendLine("            DispatchAsync(method, payload, serializer, registry, output, global::DotBoxd.Services.Streaming.RpcStreamingContext.Disabled, ct);");
            sb.AppendLine();
            sb.AppendLine("#pragma warning disable CS1998");
        ...
        switch (method.ReturnKind)
        {
            ...
            case MethodReturnKind.ValueTaskOfSubService:
            {
                ...
                if (info.AllowsNull)
                {
                    ...
                    sb.AppendLine("                    {");
                    sb.AppendLine("                        serializer.Serialize<global::DotBoxd.Services.Protocol.ServiceHandle?>(output, null);");
                    sb.AppendLine("                        return;");
                    sb.AppendLine("                    }");
                }
                ...
                sb.AppendLine("                    try");
                sb.AppendLine("                    {");
                sb.AppendLine($"                        serializer.Serialize(output, new global::DotBoxd.Services.Protocol.ServiceHandle {{ ServiceName = \"{info.ServiceName}\", InstanceId = __subId }});");
                sb.AppendLine("                    }");
                sb.AppendLine("                    catch");
                sb.AppendLine("                    {");
