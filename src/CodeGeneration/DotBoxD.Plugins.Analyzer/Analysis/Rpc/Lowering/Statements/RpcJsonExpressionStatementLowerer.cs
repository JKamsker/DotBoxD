...
internal static class RpcJsonExpressionStatementLowerer
{
    ...
    private static bool IsListAddInvocation(DotBoxDRpcJsonLowerer lowerer, InvocationExpressionSyntax invocation)
    {
        if (lowerer.Model.GetSymbolInfo(invocation).Symbol is not IMethodSymbol method ||
