...
internal static partial class ProxyGenerator
{
        ...
        bool emitClsNonCompliantAttribute)
    {
        ...
        foreach (var externAlias in service.ExternAliases.Array)
        {
            sb.AppendLine($"extern alias {externAlias};");
        }
    ...
    private static bool MethodRequiresExplicitImplementation(string methodName, string proxyName)
        => ProxyGenerationHelpers.MethodNameRequiresExplicitImplementation(methodName, proxyName) ||
           ProxyGenerationHelpers.IsCallerCancellationHelperName(
    ...
    {
        ProxyCallerCancellationHelperEmitter.Append(sb);
        sb.AppendLine("    }");
        if (!string.IsNullOrEmpty(service.Namespace))
        {
            sb.AppendLine("}");
