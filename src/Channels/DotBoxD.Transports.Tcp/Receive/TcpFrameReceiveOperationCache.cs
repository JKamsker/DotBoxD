...
/// <summary>Retains two reusable receive sources for a connection that reached shared-pool overflow.</summary>
internal sealed class TcpFrameReceiveOperationCache
{
    private const int MaxSourceCount = 2;

    private TcpFrameReceiveOperation? _first;
    private TcpFrameReceiveOperation? _second;
    ...
    public TcpFrameReceiveOperation? TryRent()
    {
    ...
    public TcpFrameReceiveOperation? TryCreate()
    {
        if (!TryReserveSource())
        {
            return null;
    ...
    public void Return(TcpFrameReceiveOperation operation)
    {
        if (Volatile.Read(ref _disposed) != 0)
        {
            return;
    ...
    private void RemoveReturnedOperationAfterDispose(TcpFrameReceiveOperation operation)
    {
        if (Volatile.Read(ref _disposed) == 0)
        {
            return;
