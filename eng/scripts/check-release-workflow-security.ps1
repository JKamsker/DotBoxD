...
if ($ciPackJob -notmatch "(?m)^\s{4}needs:\s+\[build-test,\s*rider-e2e,\s*gates,\s*coverage\]\s*$") {
    throw "CI pack job must depend on build-test, rider-e2e, gates, and coverage before producing publishable packages."
}
