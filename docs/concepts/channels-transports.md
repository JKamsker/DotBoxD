...
[`RpcPeer`](../channels/design/peer-model.md) only ever sees two interfaces:
...
ride this one substrate and are equally indifferent to it: [Services / RPC](services.md) — one C#
contract compiles to a typed proxy + dispatcher, no hand-written marshaling and no runtime reflection on
the hot path; the [query / event pipeline](kernels.md) — server-side `Where`/`Select` filtering so only
matching, projected values cross the pipe; and [pushdown](pushdown.md) — moving a loop next to the data
...
- **Symmetry falls out of the channel being duplex.** The four core
  ...
  transport property — see [peer-model](../channels/design/peer-model.md).
- **Adding a transport = implement three interfaces, change zero contracts.** The
  [WebSocket guide](../channels/websocket-setup.md) is proof by construction: it is not a shipped
...
- **Codec neutrality is the same trick applied to bytes.** `RpcPeer`/`RpcHost` take an `ISerializer`;
  ...
  Unity/IL2CPP AOT (see [unity-integration](../channels/unity-integration.md)); the `netstandard2.1`
...
|---|---|---|
| **Named pipes** (`DotBoxD.Transports.NamedPipes`) | Same machine, cross-process IPC | Separate package so TCP-only hosts take no pipe dependency; duplex, so one pipe can both serve and call. See [named-pipe transport](../channels/named-pipe-transport.md). |
| **TCP** (`DotBoxD.Transports.Tcp`) | Cross-host / over the network | Default [quick-start](../channels/quick-start.md) transport; faces untrusted networks, so `TcpConnection` ships a slow-loris defense (`DefaultFrameReadIdleTimeout`, 30s). |
| **WebSocket** (you implement it) | Browser clients, Unity WebGL, or HTTP-based connectivity | Not shipped — implement the three interfaces from the [WebSocket guide](../channels/websocket-setup.md). |
