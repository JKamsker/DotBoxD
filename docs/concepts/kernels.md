...
process: the [Query pipeline](../tutorials/event-pipeline-runlocal.md) lowers `Where`/`Select` to
server-side logic, and [Pushdown](pushdown.md) lowers a batch method that loops the host's bindings
...
you must run author-supplied logic in-process safely. Prefer a [Service](services.md) when the logic
