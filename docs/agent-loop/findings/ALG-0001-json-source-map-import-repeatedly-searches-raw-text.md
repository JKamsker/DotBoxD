...
`JsonImporter.Import` builds source spans with repeated raw-text extraction and string searches, making import cost grow poorly for large JSON IR documents.

## Evidence

- `src/DotBoxD.Kernels.Serialization.Json/JsonImporter.cs:12` first scans the full JSON with `JsonImportBudgetGuard.Validate`, then `JsonDocument.Parse` parses it again at `src/DotBoxD.Kernels.Serialization.Json/JsonImporter.cs:13`.
- `src/DotBoxD.Kernels.Serialization.Json/JsonImporter.cs:20` always constructs a `JsonSourceMap` for the whole document before reading the module.
...
Add a BenchmarkDotNet import benchmark that generates modules with 100, 1,000, and 10,000 simple statements/expressions and measures `JsonImporter.Import` time plus allocated bytes. Include a duplicate-looking literal-heavy case to expose `_spansByRawText` queue behavior.
