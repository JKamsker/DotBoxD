...
- `docs/Specs/Initial/dotboxd-sandbox-spec/spec/16-public-api.md:427` exposes `JsonImporter.Import(string json)` and `docs/Specs/Initial/dotboxd-sandbox-spec/spec/16-public-api.md:440` exposes `PluginPackageJsonSerializer.Import(string json)` as public JSON ingestion APIs.
...
- `src/DotBoxD.Kernels.Serialization.Json/JsonImporter.cs:34` hard-codes the allowed top-level module fields, and the importer repeats strict `RequireAllowedProperties` checks for functions, statements, expressions, and types throughout that file.
