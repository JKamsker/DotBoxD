...
`JsonExporter` is shipped as a public module serialization API, but the public package guidance and package consumer smoke only prove JSON import and plugin package upload. A user can discover `JsonImporter` and `PluginPackageJsonSerializer` from README/package smoke, while the module export surface is only present in the public API spec and source.
...
- `src/DotBoxD.Kernels.Serialization.Json/JsonExporter.cs:6` declares public `JsonExporter`.
- `docs/Specs/Initial/dotboxd-sandbox-spec/spec/16-public-api.md:432` lists `JsonExporter` in the JSON serialization public API.
...
- `README.md:51` says `DotBoxD.Kernels.Serialization.Json` is for `ImportJsonAsync` and `JsonImporter`, again omitting `JsonExporter`.
- `scripts/check-package-consumer-smoke.ps1:144` through `scripts/check-package-consumer-smoke.ps1:147` compile-check `JsonImporter`, `PluginPackageJsonSerializer`, and IPC types, but do not reference or call `JsonExporter.Export(...)`.

## Suggested test or smoke

Extend the package consumer smoke or add a docs-smoke fixture that constructs a small `SandboxModule`, calls `JsonExporter.Export(module)`, imports the exported JSON with `JsonImporter` or `ImportJsonAsync`, and validates that the round-tripped module can be prepared. The fixture should use the public package references and namespaces documented for consumers.

## Suggested fix direction

Update README package guidance and common namespaces to name `JsonExporter` as part of `DotBoxD.Kernels.Serialization.Json`. Add a short round-trip snippet near the JSON IR example or public API docs, then wire that snippet into release smoke so exporter package placement remains covered.
