...
// DotBoxD.Kernels surfaces: the current SDK generator only lowers IEventKernel<TEvent> kernels into plugin
...
// through the sandbox host, because filters/formulas cannot ship through the DotBoxD.Kernels
...
internal static class SimpleContractExamples
{
    ...
    {
        Console.WriteLine(
            "simple filter/formula: host-side contract design guidance only (not a DotBoxD.Kernels plugin package)");
