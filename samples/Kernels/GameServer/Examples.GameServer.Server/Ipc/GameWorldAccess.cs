...
/// <summary>
/// Declares the capability a kernel must hold to reach a world method. It lives on the SERVER impl — the
/// trusted side that owns the policy — not on the abstraction. Routing (the binding/RPC id) is derived
/// from the interface method, and the read/write effect is inferred from the implementation, so the only
/// thing worth stating is the capability string. The sandbox host bindings are generated from these.
/// </summary>
[AttributeUsage(AttributeTargets.Method)]
internal sealed class HostCapabilityAttribute(string capability) : Attribute
{
...
internal sealed class GameWorldAccess : ServerControlBase, IGameWorldAccess
{
    ...
    public GameWorldAccess(GameWorld world)
    {
        _world = world;
...
internal sealed class GameMonsterControl : ServerControlBase, IMonsterControl
{
    ...
    public GameMonsterControl(GameWorld world) => _world = world;
...
internal sealed class GameEntityControl : ServerControlBase, IEntityControl
{
    ...
    public GameEntityControl(GameWorld world) => _world = world;
