...
/// payload-clean, so the DBXS002 "contains a sub-service type" diagnostic is suppressed for
...
/// <c>UseBAsync</c> are flagged with DBXS002, because A and B each transitively contain the
...
namespace Bug.Reg.CyclePoison
{
    ...
    public void CyclicDtoReachingServiceInterface_FlagsBothMethods_NotJustTheFirst()
    {
        ...
        var subServiceDiagnostics = runResult.Diagnostics
            .Where(d => d.Id == "DBXS002" && d.GetMessage().Contains("contains a sub-service type"))
