...
public sealed class RefLikeSubServiceCompanionTests
{
    private static readonly CSharpParseOptions s_parseOptions =
        CSharpParseOptions.Default.WithLanguageVersion(LanguageVersion.Preview);

    [Fact]
    public void MetadataSubServiceWithRefLikeCompanion_ProducesDBXS002AndCompilingStub()
    {
        var referenced = CompileReference("""
            ...
            public ref struct SubProxy : ISub
            {
        ...
        var driver = GeneratorTestHelper.CreateDriver(s_parseOptions).RunGenerators(compilation);
    ...
    private static CSharpCompilation CreateCompilation(string source, MetadataReference reference) =>
        CSharpCompilation.Create(
            assemblyName: "RefLikeSubService_" + Guid.NewGuid().ToString("N"),
            syntaxTrees: [CSharpSyntaxTree.ParseText(source, s_parseOptions)],
