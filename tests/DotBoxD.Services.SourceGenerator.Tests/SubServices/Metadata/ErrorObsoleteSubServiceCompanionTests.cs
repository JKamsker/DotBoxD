...
public class ErrorObsoleteSubServiceCompanionTests
{
    private static readonly CSharpParseOptions s_parseOptions =
        CSharpParseOptions.Default.WithLanguageVersion(LanguageVersion.Latest);

    [Theory]
    [InlineData("[System.Obsolete(\"Unavailable\", error: true)]", "")]
    [InlineData("", "[System.Obsolete(\"Unavailable\", error: true)]")]
    public void RootMethodReturningErrorObsoleteMetadataCompanion_BecomesUnsupportedStub(
    ...
    private static CSharpCompilation CreateCompilation(string source, MetadataReference reference)
        => CSharpCompilation.Create(
            assemblyName: "ErrorObsoleteSubService_" + Guid.NewGuid().ToString("N"),
            syntaxTrees: [CSharpSyntaxTree.ParseText(source, s_parseOptions)],
