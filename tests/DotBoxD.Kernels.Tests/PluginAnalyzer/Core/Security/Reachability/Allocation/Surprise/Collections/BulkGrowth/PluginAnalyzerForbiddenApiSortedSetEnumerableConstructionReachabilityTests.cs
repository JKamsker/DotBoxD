...
{
    [Fact]
    public async Task Reports_reachable_sorted_set_enumerable_construction()
    {
        var diagnostics = await PluginAnalyzerCapacityTestHarness.AnalyzeAsync(
            Source("new SortedSet<byte>(Enumerable.Repeat((byte)0, int.MaxValue))"),
    ...
    [Fact]
    public async Task Does_not_report_empty_sorted_set_constructor()
    {
        var diagnostics = await PluginAnalyzerCapacityTestHarness.AnalyzeAsync(
            Source("new SortedSet<byte>()"),
            "DotBoxDPluginAnalyzerEmptySortedSetConstructorReachabilityTest");
