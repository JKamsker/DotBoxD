...
public sealed class PluginAnalyzerForbiddenApiDebugLoggingReachabilityTests
{
    private static readonly CSharpParseOptions ParseOptions =
        CSharpParseOptions.Default.WithLanguageVersion(LanguageVersion.Preview);

    [Fact]
    public async Task Reports_debug_logger_provider_writes_from_event_kernel()
    {
        var source = Source("""
        ...
        Assert.Contains("Microsoft.Extensions.Logging.Debug", diagnostic.GetMessage(), StringComparison.Ordinal);
    }
    ...
    private static string Source(string shouldHandleBody)
        => $$"""
            ...
            namespace Sample
            {
                ...
                [Plugin("debug-logging-host-api")]
                public sealed class DebugLoggingKernel : IEventKernel<string>
                {
    ...
    private static CSharpCompilation CreateCompilation(string source)
    {
        ...
        return CSharpCompilation.Create(
            "DotBoxDPluginAnalyzerDebugLoggingReachabilityTest",
            [syntaxTree],
