...
public sealed class PluginServerSetupReplayCancellationTests
{
    [Fact]
    public async Task Setup_replay_install_cancellation_does_not_publish_local_install_state()
    {
        var (_, outputCompilation) = PluginServerGenerationTestDriver.Run(Source);
        PluginServerGenerationTestDriver.AssertNoCompilationErrors(outputCompilation);

        var assembly = Load(outputCompilation);
        var probe = assembly.GetType("Sample.Plugin.SetupReplayCancellationProbe", throwOnError: true)!;
        ...
        Assert.Equal(typeof(OperationCanceledException).FullName, GetString(result, "StartExceptionType"));
        Assert.Equal(0, GetInt(result, "ConfiguredCalls"));
        Assert.Equal(typeof(InvalidOperationException).FullName, GetString(result, "UpdateExceptionType"));
        Assert.Contains("installed", GetString(result, "UpdateExceptionMessage"), StringComparison.OrdinalIgnoreCase);
    ...
    private const string Source = """
        ...
        namespace Sample.Plugin
        {
            ...
            {
                public static int ConfiguredCalls { get; private set; }

                public static void ResetConfiguredCalls() => ConfiguredCalls = 0;

                partial void OnConfigured() => ConfiguredCalls++;
            }
            ...
            {
                private readonly CancellationTokenSource startCancellation;

                public RecordingControl(CancellationTokenSource startCancellation)
            ...
            public static class SetupReplayCancellationProbe
            {
                public static async Task<object> RunAsync()
                {
                    using var cts = new CancellationTokenSource();
                    var control = new RecordingControl(cts);
                    ...
                    var startException = await CaptureAsync(() => server.StartAsync(cts.Token).AsTask());
                    ...
                    return new ProbeResult(
                        ...
                        RemotePluginServer.ConfiguredCalls,
