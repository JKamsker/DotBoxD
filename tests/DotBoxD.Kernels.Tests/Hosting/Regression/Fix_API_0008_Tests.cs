...
/// <see cref="DiagnosticDescriptor.HelpLinkUri"/> that deep-links to its actionable public
...
public sealed class Fix_API_0008_Tests
{
    ...
    [Fact]
    public void Forbidden_host_api_rule_links_to_public_reference()
    {
    ...
    [Fact]
    public void Live_setting_type_rule_links_to_public_reference()
    {
    ...
    public void Release_tracked_rule_ids_are_documented_in_the_referenced_files()
    {
        ...
        // Release tables remain the machine-readable shipped/unshipped inventory even though
        // consumer help links now point at the actionable documentation page.
        Assert.Contains("DBXK001", shipped);
        Assert.Contains("DBXK020", shipped);
        Assert.Contains("DBXK100", unshipped);
    }

    private static void AssertHelpLink(DiagnosticDescriptor rule)
    {
        Assert.False(
        ...
        Assert.EndsWith("#" + rule.Id.ToLowerInvariant(), rule.HelpLinkUri, StringComparison.Ordinal);
    }
