...
{
    [Fact]
    public async Task Package_preparation_waits_for_late_breakpoint_synchronization()
    {
        const string source = "return damage;";
        const string path = "/source/AcknowledgedLateChain.cs";
        await using var bridge = PluginDebugBridge.Start(new PluginDebugBridgeOptions
        {
            DebuggerWaitTimeout = TimeSpan.FromSeconds(2),
            SourceReader = _ => Encoding.UTF8.GetBytes(source)
        });
        var control = new DelayedBreakpointControl();
        ...
        using var request = BreakpointRequest(path);
        await handler.HandleAsync(request.RootElement, CancellationToken.None);
        _ = await client.SendAsync("configurationDone", null, CancellationToken.None);
        var node = SandboxNodeMap.Create(package.Module).Nodes[0];
        var document = KernelDebugDocument.FromSource("acknowledged-late-chain", path, source);
        var mapped = package with
        {
        ...
        var preparation = bridge.PreparePackageAsync(mapped).AsTask();
        await control.BreakpointConfigurationStarted.Task.WaitAsync(TimeSpan.FromSeconds(2));
        Assert.False(preparation.IsCompleted);

        control.AllowBreakpointConfiguration.TrySetResult();
    ...
    private static JsonDocument BreakpointRequest(string path)
        => JsonDocument.Parse(JsonSerializer.SerializeToUtf8Bytes(new
        {
            seq = 1,
            type = "request",
    ...
    private sealed class DelayedBreakpointControl : IPluginDebugControlRpcService
    {
        public string SessionToken { get; } = Convert.ToHexStringLower(Guid.NewGuid().ToByteArray()) +
            Convert.ToHexStringLower(Guid.NewGuid().ToByteArray());

        public TaskCompletionSource BreakpointConfigurationStarted { get; } =
            new(TaskCreationOptions.RunContinuationsAsynchronously);

        public TaskCompletionSource AllowBreakpointConfiguration { get; } =
            new(TaskCreationOptions.RunContinuationsAsynchronously);

        public async ValueTask<byte[]> ExchangeAsync(byte[] message, CancellationToken cancellationToken = default)
        {
            var request = PluginDebugProtocol.Decode(message, 1024 * 1024);
            if (request.Kind == PluginDebugCommands.SetBreakpoints &&
                request.Payload.GetProperty("breakpoints").GetArrayLength() > 0)
            {
                BreakpointConfigurationStarted.TrySetResult();
                await AllowBreakpointConfiguration.Task.WaitAsync(cancellationToken);
            }
            ...
            return PluginDebugProtocol.Encode(response, 1024 * 1024);
        }
    }
}
