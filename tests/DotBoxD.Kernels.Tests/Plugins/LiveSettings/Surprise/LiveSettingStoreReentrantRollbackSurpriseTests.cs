...
public sealed class LiveSettingStoreReentrantRollbackSurpriseTests
{
    ...
    [Fact]
    public void Failed_batch_rejects_a_reentrant_update_during_rollback()
    {
        var side = new LiveValue<int>("Side", 1);
        LiveSettingStore? store = null;
        var first = new CallbackLiveSetting("First", 1, value =>
        ...
        Assert.Contains(exception.Flatten().InnerExceptions, error => error.Message == "second failed");
        Assert.Contains(
            exception.Flatten().InnerExceptions,
            error => error.Message == "Live settings cannot be updated during rollback.");
        Assert.Equal(1, store.Get<int>("Side"));
        Assert.Equal(1, store.Get<int>("First"));
        Assert.Equal(2, store.Get<int>("Second"));
    }
    ...
    [Fact]
    public void Failed_batch_reports_a_setting_that_rejects_restoration()
    {
        var first = new RestorationRejectingLiveSetting("First", 1);
        var second = new ThrowingLiveSetting("Second", 2, 20);
        var store = new LiveSettingStore([first, second]);
        ...
        Assert.Contains(exception.Flatten().InnerExceptions, error => error.Message == "second failed");
        Assert.Contains(exception.Flatten().InnerExceptions, error => error.Message == "restoration failed");
        Assert.Equal(10, store.Get<int>("First"));
        Assert.Equal(2, store.Get<int>("Second"));
    }
    ...
    private sealed class RestorationRejectingLiveSetting : ILiveSetting
    {
        private readonly int _initialValue;
        private int _value;

        public RestorationRejectingLiveSetting(string name, int initialValue)
        {
            Name = name;
            Definition = new LiveSettingDefinition(name, "int", initialValue);
        ...
        {
            var typedValue = Assert.IsType<int>(value);
            if (_value != _initialValue && typedValue == _initialValue)
            {
