...
public sealed class RegistrationAccumulatorGeneratedMemberCollisionTests
{
    ...
    public void Root_child_named_like_flush_helper_reports_dbxk100()
    {
        ...
        Assert.Contains(
            diagnostics,
            d => d.Id == "DBXK100" &&
                 d.GetMessage().Contains("FlushAsync", StringComparison.Ordinal) &&
                 d.GetMessage().Contains("collides", StringComparison.Ordinal));
        Assert.DoesNotContain(diagnostics, d => d.Id is "CS0102" or "CS0111");
    }
