...
public sealed class ServerExtensionClientCancellationEncodingSurpriseTests
{
    private const string InvalidUtf16 = "payload-\uD800";

    [Fact]
    public async Task Service_backed_generated_client_observes_pre_canceled_token_before_encoding_arguments()
    {
        ...
        var valueTask = assembly.GetType("Sample.Probe", throwOnError: true)!
            ...
            .Invoke(null, [control, InvalidUtf16, cts.Token])!;
    ...
    [Fact]
    public void Direct_generated_client_observes_pre_canceled_token_before_encoding_arguments()
    {
    ...
    private const string ServiceBackedSource = """
        ...
        {
            ValueTask<int> EchoAsync(string payload, CancellationToken cancellationToken = default);
        }
        ...
        {
            public int Echo(string payload, HookContext ctx) => 1;
        }
