...
public sealed class NamedPipeClientTransportLifecycleTests
{
    [Fact]
    ...
    public async Task AcceptAsync_Throws_WhenSecondPendingAcceptStarted()
    {
        ...
        await Assert.ThrowsAnyAsync<OperationCanceledException>(() => firstAccept.WaitAsync(TransportTimeout));
    }
    ...
    public async Task AcceptAsync_Throws_WhenStoppedWhilePending()
    {
        ...
        await Assert.ThrowsAnyAsync<OperationCanceledException>(() => acceptTask.WaitAsync(TransportTimeout));
    }
    ...
    public async Task AcceptAsync_AcceptsMultipleConnectionsSequentially()
    {
        ...
        for (var i = 0; i < 3; i++)
        {
            ...
            await client.ConnectAsync().WaitAsync(TransportTimeout);
            await using var serverConnection = await acceptTask.WaitAsync(TransportTimeout);
...
public sealed class SingleConnectionTransportCoverageTests
{
    ...
    public async Task ConnectAsync_CompletesImmediately_AndExposesConnection()
    {
        ...
        await transport.ConnectAsync().WaitAsync(TransportTimeout);
...
public sealed class SingleConnectionServerTransportCoverageTests
{
    ...
    public async Task AcceptAsync_ReturnsConnection_OnFirstCall()
    {
        ...
        var accepted = await server.AcceptAsync().WaitAsync(TransportTimeout);
    ...
    public async Task AcceptAsync_Blocks_OnSecondCall_UntilStopped()
    {
        ...
        var first = await server.AcceptAsync().WaitAsync(TransportTimeout);
