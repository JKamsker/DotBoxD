...
public sealed class RpcPeerPublicGuardCoverageTests
{
    ...
    public async Task ProvideDispatcher_AfterRemoteClose_ThrowsConnectionClosed()
    {
        ...
        channel.Enqueue(DotBoxD.Services.Buffers.Payload.Empty);
    ...
    private static DotBoxD.Services.Buffers.Payload ServerStatusResponseFrame(
        ISerializer serializer, int messageId, string version)
    {
        ...
        return DotBoxD.Services.Protocol.MessageFramer.FrameMessage(
            serializer,
            messageId,
            DotBoxD.Services.Protocol.MessageType.Response,
            new DotBoxD.Services.Protocol.RpcResponse { MessageId = messageId, IsSuccess = true },
