...
namespace DotBoxD.Services.Tests.Cov.BugFixes;
...
public sealed class BugFixRegressionTests
{
    ...
    {
        var pipeName = "dotboxd-bugfix-" + Guid.NewGuid().ToString("N");
    ...
    {
        var client = new NamedPipeClientTransport("dotboxd-bugfix-" + Guid.NewGuid().ToString("N"));
