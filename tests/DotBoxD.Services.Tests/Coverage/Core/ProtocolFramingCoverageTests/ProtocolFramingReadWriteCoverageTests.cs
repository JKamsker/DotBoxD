...
public sealed class ProtocolFramingReadWriteCoverageTests
{
    [Theory]
    [InlineData(0)]
    [InlineData(1)]
    ...
    public async Task ReadMessageAsync_DeclaredLengthBelowHeader_ThrowsInvalidData()
    {
        ...
        var ex = await Assert.ThrowsAsync<InvalidDataException>(
            () => MessageFramer.ReadMessageAsync(stream).AsTaskWithTimeout(FramingTimeout));
    ...
    public async Task WriteThenReadMessageAsync_PreservesIdTypeAndBody(MessageType type)
    {
        ...
        // Act
        await MessageFramer.WriteMessageAsync(stream, messageId, type, payload).AsTaskWithTimeout(FramingTimeout);
        stream.Position = 0;
        var result = await MessageFramer.ReadMessageAsync(stream).AsTaskWithTimeout(FramingTimeout);
