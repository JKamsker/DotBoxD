...
public sealed class EndToEndRpcScenarioTests
{
    [Fact]
    ...
    public async Task UnknownService_OverTcp_ThrowsServiceNotFound()
    {
        ...
        await using var h = await EndToEndTransportHarness.StartTcpAsync(_ => { });

        var ex = await Assert.ThrowsAsync<RemoteServiceException>(
            () => h.Game.GetServerStatusAsync().WaitAsync(EndToEndTimeout));
    ...
    public async Task UnknownMethod_OnKnownService_ThrowsMethodNotFound()
    {
        ...
        {
            var ex = await Assert.ThrowsAsync<RemoteServiceException>(
                () => client.InvokeAsync<ServerStatus>("IGameService", "NoSuchMethod").WaitAsync(EndToEndTimeout));
        ...
        try
        {
            var ex = await Assert.ThrowsAsync<RemoteServiceException>(
                () => client.InvokeOnInstanceAsync<ServerStatus>(
                    "IGameService", "no-such-instance", "GetServerStatusAsync").WaitAsync(EndToEndTimeout));
    ...
    public async Task ClientCancellation_OverInMemoryPipe_CancelsServerSideHandler()
    {
        ...
        try
        {
            ...
            await service.Entered.Task.WaitAsync(EndToEndTimeout);

            cts.Cancel();

            await Assert.ThrowsAnyAsync<OperationCanceledException>(() => call.WaitAsync(EndToEndTimeout));
            // The server-side handler must observe cancellation on its own CancellationToken.
            await service.Canceled.Task.WaitAsync(EndToEndTimeout);
        }
        finally
        {
            await client.DisposeAsync();
    ...
    public async Task LargePayload_OverTcp_RoundTripsIntact()
    {
        ...
        var registered = await h.Game.RegisterPlayerAsync(bigName).WaitAsync(EndToEndTimeout);
        ...
        var fetched = await h.Game.GetPlayerStateAsync(new PlayerId { Id = registered.PlayerId }).WaitAsync(EndToEndTimeout);
    ...
    public async Task LargePayload_OverByteFragmentedPipe_RoundTripsIntact()
    {
        ...
        try
        {
            ...
            var registered = await game.RegisterPlayerAsync(bigName).WaitAsync(EndToEndTimeout);
            Assert.Equal(bigName, registered.Name);
        }
        finally
        {
    ...
    public async Task InvokeAfterServerDisconnect_OverTcp_ThrowsConnectionError()
    {
        ...
        await host.StartAsync().WaitAsync(EndToEndTimeout);
        ...
        await clientTransport.ConnectAsync().WaitAsync(EndToEndTimeout);
        ...
        try
        {
            ...
            await host.DisposeAsync().AsTask().WaitAsync(EndToEndTimeout);
            ...
            await Assert.ThrowsAnyAsync<ServiceException>(
                () => InvokeUntilFailsAsync(game).WaitAsync(EndToEndTimeout));
        }
        finally
        {
            await client.DisposeAsync();
