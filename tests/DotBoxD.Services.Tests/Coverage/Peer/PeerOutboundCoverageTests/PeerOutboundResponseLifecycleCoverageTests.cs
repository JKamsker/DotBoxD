...
public sealed class PeerOutboundResponseLifecycleCoverageTests
{
    [Fact]
    public async Task InvokeAsync_ResponseForUnknownMessageId_IsIgnored_RequestStillTimesOut()
    {
        ...
        var ex = await Assert.ThrowsAsync<ServiceTimeoutException>(() => call.WaitAsync(PeerOutboundTimeout));
    ...
    public async Task InvokeAsync_DuplicateResponseForSameId_SecondIsIgnored()
    {
        ...
        Assert.Equal("first", await call.WaitAsync(PeerOutboundTimeout));
        ...
        Assert.Equal("second", await second.WaitAsync(PeerOutboundTimeout));
    }
    ...
    public async Task InvokeAsync_ResponseEnvelopeMessageIdMismatch_FaultsRequestWithProtocolException()
    {
        ...
        var ex = await Assert.ThrowsAsync<ServiceProtocolException>(() => call.WaitAsync(PeerOutboundTimeout));
    ...
    public async Task InvokeAsync_TimesOut_WhenNoResponseArrives()
    {
        ...
        var ex = await Assert.ThrowsAsync<ServiceTimeoutException>(
            () => peer.InvokeAsync<int, string>(Service, Method, request: 1).WaitAsync(PeerOutboundTimeout));
    ...
    public async Task InvokeAsync_TokenCancelled_FaultsRequest_AndSendsCancelFrame()
    {
        ...
        await channel.WaitForSentFrameIdsAsync(1, PeerOutboundTimeout);
        cts.Cancel();
        ...
        var cancel = await channel.WaitForFrameOfTypeAsync(MessageType.Cancel, PeerOutboundTimeout);
    ...
    public async Task InvokeAsync_PreCancelledToken_ThrowsWithoutReservingASlot()
    {
        ...
        await Assert.ThrowsAsync<OperationCanceledException>(
            () => peer.InvokeAsync<int, string>(Service, Method, request: 1, cts.Token).WaitAsync(PeerOutboundTimeout));
        ...
        Assert.Equal("ok", await call.WaitAsync(PeerOutboundTimeout));
    }
    ...
    public async Task Dispose_WithPendingRequest_FaultsItWithConnectionClosed()
    {
        ...
        var ex = await Assert.ThrowsAsync<ServiceConnectionException>(() => call.WaitAsync(PeerOutboundTimeout));
    ...
    public async Task CloseAsync_WithPendingRequest_FaultsItWithConnectionClosed()
    {
        ...
        await Assert.ThrowsAsync<ServiceConnectionException>(() => call.WaitAsync(PeerOutboundTimeout));
    ...
    public async Task InvokeAsync_AfterSendFailure_SlotIsReleasedAndPeerStillUsable()
    {
        ...
        Assert.Equal("recovered", await call.WaitAsync(PeerOutboundTimeout));
    }
    ...
    public async Task InvokeAsync_AfterRemoteClose_FaultsWithConnectionClosed_AndSenderRejectsLateSend()
    {
        ...
        var ex = await Assert.ThrowsAsync<ServiceConnectionException>(() => inFlight.WaitAsync(PeerOutboundTimeout));
        Assert.Contains("closed", ex.Message);
        ...
        await WaitUntilAsync(() => !peer.IsConnected, PeerOutboundTimeout);
