...
public sealed class StreamFrameReceiveOperationCacheTests
{
    [Fact]
    public void CreationAndReturn_AreBoundedToTwoSources()
    {
        var cache = new StreamFrameReceiveOperationCache();
        var first = Assert.IsType<StreamFrameReceiveOperation>(cache.TryCreate());
        var second = Assert.IsType<StreamFrameReceiveOperation>(cache.TryCreate());
    ...
    public void Dispose_DropsIdleAndLateReturningSources()
    {
        ...
        cache.Return(first);
        cache.Return(second);
        var leased = Assert.IsType<StreamFrameReceiveOperation>(cache.TryRent());
    ...
    [Fact]
    public async Task DisposeRacingReturn_NeverLeavesAReachableSource()
    {
        for (var iteration = 0; iteration < 512; iteration++)
        {
            var cache = new StreamFrameReceiveOperationCache();
            var operation = Assert.IsType<StreamFrameReceiveOperation>(cache.TryCreate());
            using var start = new Barrier(participantCount: 3);
            var returning = Task.Run(() =>
            {
                start.SignalAndWait();
            ...
            start.SignalAndWait();
            await Task.WhenAll(returning, disposing);
    ...
    public async Task ConcurrentRentAndReturn_NeverDoubleLeasesOrCreatesThirdSource()
    {
        ...
        cache.Return(first);
        cache.Return(second);
        var leased = new ConcurrentDictionary<StreamFrameReceiveOperation, byte>();
        ...
        var workers = Enumerable.Range(0, 8).Select(workerIndex => Task.Run(() =>
        {
            ...
            for (var iteration = 0; iteration < 2_000; iteration++)
            {
                ...
                Assert.True(leased.TryAdd(operation, 0), "A Stream receive source was leased twice.");
                Thread.Yield();
