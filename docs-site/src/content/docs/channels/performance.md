...
- The caller peer opts into pooled `ValueTask<T>` responses. Finite per-call timeouts are supported.
...
Passing a cancellable caller token keeps safety but intentionally falls back to the allocating
`Task<T>` path.
