...
| `EnableLowAllocationValueTaskInvocations` | `bool` | `false` | Opts generated unary `ValueTask` and `ValueTask<T>` proxy calls into the pooled response-source path. Finite `RequestTimeout` values are supported; a call with a cancellable token intentionally uses the task-backed path |
...
| `InboundQueueCapacity` | `int?` | 1024 | Max queued inbound requests (bounded read-side backpressure). `null` dispatches immediately and does not cap concurrent dispatch work - trusted peers or externally bounded transports only |
...
see [Performance Hot Paths](/channels/performance/).
...
See [Generated Service Registry](https://github.com/JKamsker/DotBoxD/blob/main/docs/channels/generated-service-registry.md) for examples and assembly-scope details.
